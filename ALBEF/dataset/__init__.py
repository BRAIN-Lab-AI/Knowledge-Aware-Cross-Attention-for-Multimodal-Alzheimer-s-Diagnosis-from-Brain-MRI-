...
def identity(x):
    """Helper function to replace lambda for pickling support"""
    return x
...
def create_dataset3d(dataset, config):
    ...
    pretrain_transform_pet = RandomAugment3D(
        pre_augs=[],
        rand_augs=[
            tio.RandomAffine(scales=(0.8, 1.2), degrees=20),
            tio.RandomElasticDeformation(),
            tio.RandomNoise(mean=1, std=0.1),
            tio.Lambda(identity), 
        ],
        post_augs=[],
        num_transforms=1
    ...
    if dataset=='pretrain':
        # FIX: Pass config['train_file'] as ann_file
    ...
    # Legacy blocks just in case
    if dataset=='feat_extract_train':
         dataset = pretrain_dataset3d(ann_file=config['train_file'], transform_pet=None, transform_mri=None)
         return dataset
    if dataset=='feat_extract_val':
         dataset = pretrain_dataset3d(ann_file=config['val_file'], transform_pet=None, transform_mri=None)
         return dataset
    if dataset=='feat_extract_test':
         dataset = pretrain_dataset3d(ann_file=config['test_file'], transform_pet=None, transform_mri=None)
         return dataset
