...
def train(model, data_loader, optimizer, epoch, warmup_steps, device, scheduler, config):
    ...
    t = time.time()
    for i, (mri, pet, _) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_mlm, loss_ita, loss_itm = model(mri, pet, alpha = alpha)  
...
def main(args, config):
    ...
    t = time.time()
    datasets = [create_dataset3d('pretrain', config)]
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        # FIX: Only run barrier if distributed processing is actually enabled
        if args.distributed:
            dist.barrier()  
